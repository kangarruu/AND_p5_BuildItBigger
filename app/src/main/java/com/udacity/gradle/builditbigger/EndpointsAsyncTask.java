...
public class EndpointsAsyncTask extends AsyncTask<Context, Void, String> {
    ...
    protected String doInBackground(Context... params) {
        if(myApiService == null) {
            MyApi.Builder apiBuilder = new MyApi.Builder(new NetHttpTransport(),
                    new AndroidJsonFactory(), null)// options for running against local devappserver
                    // - Used personal IP address, switch to 10.0.2.2 for localhost's IP address in Android emulator
                    // - turn off compression when running against local devappserver
                    .setRootUrl("http://192.168.1.108:8080/_ah/api/")
